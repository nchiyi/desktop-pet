...
- 角色在桌面遊走（依移動模式設定）
- 每 30~120 秒隨機觸發偶發動畫
- 偶爾頭頂出現閒聊氣泡（可關閉）；內容來自 app 內建語句庫（`assets/idle_phrases.toml`），每次隨機抽一句
...
| 固定區域 | 限制在上 / 下 / 左 / 右 某一邊緣帶狀區域移動（上下 = 螢幕高度 10%，左右 = 螢幕寬度 10%）|
...
- CLI 執行路徑（自動偵測 / 手動指定）
- 連線測試
- 預設回覆語言（app 在每則 prompt 前自動附加 `請用{language}回覆：`）
...
- 每個 session 保留完整對話記錄（local JSON）
- 儲存位置：`%APPDATA%\DesktopPet\` (Windows) / `~/Library/Application Support/DesktopPet/` (macOS)
- `characters/` 目錄同樣存放於此路徑下，隨 app 資料一起管理
